...
def process_guess(guess, word):
    ...
    global guessed_letters      # 추측된 단어를 저장할 변수
    
    lives_remaining -= 1        # 기회가 1 줄어듬
    guessed_letters += guess    # 컴이 추측된 단어에 게이머가 입력한 글자가 추가됨
    if guess == word:
        return True             # 프로그램 종료 - print("You win! Well done")를 즉시 실행
...
def play():
    ...
    while True:
        ...
        if process_guess(guess, word):  # 주측에서 맞췄을
